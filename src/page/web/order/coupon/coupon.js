...
export class Coupon extends BaseVue {
    headNav = [{ name: "可使用优惠券", number: 0 }, { name: "不可使用优惠券", number: "" }];
    headIndex = 0;
    couponList = [];
    _$service;
    couponState = 0;
   ...
   initPage(){
       ...
       let couponParam =JSON.parse(sessionStorage.getItem("couponParam"));
       let _param = {
           limit:10,
           page:1,
    ...
    chooseCoupon(item, index){
        if(this.id == item.userCouponId){
            ...
            sessionStorage.setItem("couponDetail",JSON.stringify(item));
        }
    ...
    goOrderSubmit(){
        ...
        this.$router.replace({ path:"order_submit",query:_query});
    }
