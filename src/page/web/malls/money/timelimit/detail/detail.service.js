...
export default (store) => {
    ...
    return {
        ...
        queryWdInfo(shopId) {
            return store.dispatch('CHECK_WD_INFO', shopId);
        },
        getWdInfo(data) {
        ...
        queryGoodsStock(data) {
            return q("api/limited/q_user_limited_stock", data);
        },
        //查询限时购剩余时间
        queryTime(data){
            return q("api/limited/q_limited_goods_time",data);
        },
        queryUserBuyNum(data){
            return q("api/order/q_user_buy_number",data);
        },
        queryPeriodState(data){
            return q("api/limited/q_limited_state", data);
        }
    }
}
