...
import shopChiefService from './shop_chief.service';
import './shop_chief.scss';

@Component({
    template: require('./shop_chief.html'),
      components: {
...
export class ShopChief extends BaseVue{
    // 组件方法也可以直接声明为实例的方法
    _$service;
    shopkeeper = {};                      //店铺信息
    classifyNav = [
    ...
    mounted() {
        // 注册服务
        this._$service = shopChiefService(this.$store);
    }
    ...
    */
    async queryClassifyNav(firstLoad = true) {
        ...
        this.classifyNav = [arr, ..._classify.data];
        // if (firstLoad) {
    ...
    */
    ...
    async swichTabGoods(id, index) {
        ...
        this.$router.replace({ path: "shop_chief", query: { classify: id } });
    ...
    goSearch() {
        ...
        this.$router.push({ path: "search", query: { origin: 'shop_chief' + typeId } });
    }
    //加入购物车
    QuicklyJoinTheShoppingCart(item) {
        finishToCar(this.$store).finishToCar(this, item,"shop_chief",0);
    }
    //详情
    toDetail(goodsId) {
        this.$router.push({ path: "goods_detail", query: { goodsId: goodsId, isAgent:false} })
    }
    itemHeight = 0;
    _homeHeight = 0;
