<template>
    <div class="calendar">
        <div class="calendar-tools" v-if="showTitle">
        ...
        <thead>
            <tr>
                <td v-for="(week, i) in weeks" class="week" :key="i">{{week}}</td>
            </tr>
        </thead>
        <tbody>
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}" :key="k1">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)" :key="k2">
                ...
                <!-- 扩展签到插件  -->
                <div class="sign-day-icon" v-if="signDays.includes(child.day) && !child.disabled">
                    <img src="/static/images/daysign/qiandaobiaoqian.png">
                </div>
            </td>
        </tr>
        </tbody>
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="(y, i) in years" @click.stop="selectYear(y)" :class="{'active':y==year}" :key="i">{{y}}</span>
        </div>
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (this.range) { // 范围
                    ...
                    if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(seletSplit[2]) == i) {
                        ...
                        if(this.showToDay){
                            temp[line][this.today[1]]._dayStr = '今天';
                        }
                    }
                     // 没有默认值的时候显示选中今天日期
